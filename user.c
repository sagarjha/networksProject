...
int receiveFile(int socketD, char* fileName, int fileNameSize);
...
int main(int argc, char* argv[])
{
  ...
  if(argc != 6)
    {
      printf("Usage: <executable> <0 for send, 1 for retrieve> <filename> <directory> <remoteIP> <remote Port Number>\n");
      return -1;
    }
  ...
  */
  ...
  if(option == 0)
	...
	return -1;
	  ...
	  printf("Connection Received from IP %s, Port# %d\n", inet_ntoa(remoteAddress.sin_addr), ntohs(remoteAddress.sin_port));
	  receiveFile(requestSocket, fileName, getSize(fileName));
	  //shutdown initiated, not necessary
	  shutdown(requestSocket, 2);
...
int sendFile(int socketD, char fileName[50], int fileNameSize)
{
  ...
  char justName[50];
  extractFileName(justName, &fileNameLength, fileName, fileNameSize);

  printf ("%s %d\n",fileName,fileNameLength);
  /*sending the file name size*/
  nbytes = write(socketD, &fileNameLength, sizeof(int));

  /*sending the file name*/
  nbytes = write(socketD, justName, fileNameLength);
...
void extractFileName(char* justName, int* fileNameLength, char* fileName, int fileNameSize) {
  *fileNameLength = fileNameSize;
  memcpy (justName, fileName, fileNameSize);
}
